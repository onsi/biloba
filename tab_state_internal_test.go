...
var copiedBilobaFields = map[string]string{
	...
	// Both of these are slices, which a view copies - they are safe only because every access goes
	// through b.root.  See the note on Biloba.artifacts.
	"artifacts":         reasonRootOwned,
	"visualComparisons": reasonRootOwned,

	"realistic":    reasonViewFlag,
	"immediate":    reasonViewFlag,
	"viewportOnly": reasonViewFlag,
}
