...
`one`/`poll` both stamp a **`found`** boolean onto every response (via `withFound`) reporting whether the selector resolved on that attempt, independent of whether the operation then succeeded. The poll-trajectory recorder reads it so a failure can tell "matched, then stopped matching" (a replaced node) from "never matched", and `HaveProperty`'s presence-only form branches on it to rebuild the missing-selector error that a `poll(...)` response swallows. So it is not purely diagnostic: a `poll(...)` handler that stops reporting `found` changes which not-found errors reach the user. You get it for free by using the combinators — don't hand-roll a handler that skips them.
...
`engine/biloba.js` is a generated copy that `bilobad` embeds. After editing `biloba.js`, run `go generate ./engine`; otherwise the engine suite's drift spec fails (see the `biloba-testing` skill).
...
- **An existence-only form should capture too — teach the handler to return the value it already had in hand.** A "is it there?" matcher usually *has* to touch the value to answer, so make the round-trip carry it back and `.Capture` works uniformly across both forms of the same matcher. `HaveProperty(name)` (no expected) calls `getPropertiesP` rather than the boolean `hasProperty`, keeping presence-not-truthiness semantics while making the value available; `HaveAttribute(name)` likewise; `eachHasProperty` returns `{count, values}` — `count` drives the fail-on-empty message, `values` is the very same `[]any` the value-matching form produces — so `EachHaveProperty(name)` (and the no-arg `EachHaveInnerText()`/`EachHaveTextContent()` that route through it) hand back the whole slice; and the geometry matchers `HaveGapBetween`/`HaveOffsetTopWithin`/`HaveOffsetLeftWithin` follow the same shape for their zero-arg form (presence of a computable `BoxDelta`/offset, not a value assertion). **A `*ValueMatcher` whose `Capture` leaves the target at zero is a bug, not a signature convenience** — if you find yourself about to document that a form "returns a ValueMatcher for consistency but captures nothing," go make the JS return the value instead.
- **A variadic `expected ...any` matcher must branch on `len(expected) == 0` explicitly — never default the missing case to a `nil` sub-matcher.** `matcherOrEqual(nil)` produces `gomega.BeNil()`, which asserted against a struct or a `float64` can never pass — so the zero-arg form compiles, reads correctly, and is permanently broken. That shape shipped on `HaveGapBetween`/`HaveOffsetTopWithin`/`HaveOffsetLeftWithin` (issue #13). Give `len(expected) == 0` its own branch with real presence semantics, the way every other variadic matcher here does; don't reject the call with a `Fatalf`.
...
- **A query matcher with its own builder chain grows its own `Capture`.** `CookieMatcher` (`cookie_matchers.go`) carries `.WithValue/.WithPath/.WithSecure/…`, so it can't be a `*ValueMatcher`; it defines `Capture(&cookie)` that copies the matcher like `ValueMatcher.Capture` does, calls the same `decodeCapture`, and composes with the refinements in either order (`withField` carries `captureTarget` forward onto each refinement). Follow that shape for a new builder-style matcher rather than forcing it into `ValueMatcher`.
...
```
`SetValue`/`SetProperty`/`DragTo`/`ScrollWheel`/`Focus`/`Blur` etc. follow the same fork directly in `dom.go`. `HaveProperty(property, expected ...any)` (existence-only vs value-matching) is a pure matcher, so it has no fork.
...
**Split the two cases deliberately: missing → error, present-but-not-ready → silent retry.** The geometry handlers are the reference. They're built on `one(...)` (or hand-rolled `sel()` + `notFound(...)` for the two-selector probes) rather than `poll(...)`, so a **missing** element returns an error — which under `Eventually` is still just a failed attempt, but keeps `ShouldNot(b.BeInViewport())` from passing instantly against an element that never rendered. The **degenerate box** stays `{success:false}` with no error, so the positive direction polls through late layout. Same rule elsewhere: `isChecked` errors when the node has no `checked` property at all (the "I selected the label, not the input" bug) instead of coercing `undefined` to `false`. Whenever you write a `poll(...)`-backed handler, ask what a negated assertion on it would do against an empty page.

**And say what the negation should be instead.** An error is never a satisfied assertion in *either* direction, so making missing-is-an-error also breaks the legitimate wait-for-teardown spec (`Eventually(sel).ShouldNot(b.BeInViewport())` on a toast that gets removed). Whenever you flip a matcher this way, the docs/skills/changelog entry must name the replacement idiom — assert disappearance with the matchers that are *about* existence, `ShouldNot(b.Exist())` / `Should(b.HaveCount(0))` — not just the vacuous pass it fixed.

**The shape of a geometry getter.** Each is a `Get*` following the pattern above plus a `Have*`-matcher counterpart: one decodes the struct into a closure var, the other runs a caller-supplied sub-matcher against the decoded value. Both turn the JS map into a typed struct (`Box`/`ScrollOffset`/`BoxDelta`) through a small `newBox`/`newScrollOffset`/`newBoxDelta`, à la `newProperties`. **Element-to-element** probes (`relativeBoxesP`, `documentOrderP`) take a *second* selector — Go encodes it with `encodeSelector` and passes the encoded string so the JS `sel()` resolves it directly, and both elements are read in ONE eval so the relation is judged at a single layout instant; splitting it into two getters loses that atomic frame. They back the relational matchers (`BeAbove`/`Encloses`/`Overlaps`/`BePrecededBy`/…) plus the `GetGapBetween`/`HaveGapBetween` dual. `BeInViewport` is a single-selector matcher reading the rect against `window.innerWidth/Height`.

**Recording for the poll-trajectory artifact (`probe_trajectory.go`) takes TWO calls, not one.** A read that observes a value and compares it calls `b.recordProbe(probeKey(method, sel), value)` in the matcher body, **and** its matcher is wrapped in `b.probing(method, ...)` at construction — with the *same* `method` string, so `probeKey` lines up. Both are no-ops unless `BilobaConfigPollTrajectory` is on (the default).

`recordProbe` alone renders nothing, deliberately. The recorder keeps one series per tab, so "the current series" and "the series that failed" are different things: without the mark, a read that passed and moved on would be attached to whatever failed next, complete with a `flat (the page is not re-evaluating this probe)` diagnosis over a value nobody was waiting on. `probing` marks the series at the one moment Gomega reveals which assertion failed, by asking that matcher for its `FailureMessage`/`NegatedFailureMessage`. Only a marked series renders. So: **add a `recordProbe` without a `probing` wrap and your trajectory is dead weight; wrap with a mismatched `method` and it is dead weight too.** Cover it with a spec that drives the read to a real timeout (`NewWithT(gt)` + a short `WithTimeout`) and asserts on `b.PollTrajectoryForTest()` — `probe_trajectory_test.go` is the model.

Record only where there is a *value trajectory* to show: a matcher that read a value and rejected it (`HaveProperty`, `HaveCount`, `HaveComputedStyleNumeric`, `EvaluateTo`, …). A getter whose failure mode is "no value at all" (the element never appeared, the property was never defined) has nothing to plot — the ordinary failure message and the `AllowMissing` enrichment already say what happened, and a one-row trajectory over a value the poll never saw is a confident wrong answer. One-shot mutations (`b.Run`/`b.RunAsync`) record nothing at all; `b.EvaluateTo`/`b.GetJSValue` are the polling forms that do.
...
`b.Realistic()`, `b.WithTimeout(d)`, `b.WithPolling(d)`, `b.WithContext(ctx)` and `b.Immediate()` are **shallow copies** of the `Biloba` struct (`nb := *b; nb.flag = true`). So anything a method reads or writes on the tab has to be reachable through a copy:
...
- the not-found / timeout path: the fully-applied form polls, so assert it with a short `b.WithTimeout(...)` and `ExpectFailures(ContainSubstring("Timed out after"))` rather than an exact message; add an `Immediate()` spec when you want the fail-fast message,
...
Run `make test`, and `make test-all` since DOM interaction can differ between the two Chrome lanes. If you changed `biloba.js`, regenerate `engine/biloba.js` and run `make driver-test` too. The full set of canonical invocations, and when to reach for each, is in the `biloba-testing` skill.

## Step 4 — docs, skills, and changelog

- Update the narrative docs in `docs/index.md` (this is the source of truth for usage; godoc only links to it), and `docs/vitest.md` if the TypeScript client exposes the behavior. User-facing behavior also needs a terse godoc comment ending in a `Read https://onsi.github.io/biloba/#...` link.
- **Update the skills in the same change** when you add or alter a method family, an option, or a convention: the repo skills under `.claude/skills/`, and the shipped Go skills under `plugins/biloba-go/skills/` (plus `plugins/biloba-vitest/skills/` if the TypeScript client exposes the behavior). They go stale silently; when in doubt, update.
- Add a brief entry under `## Unreleased` in `CHANGELOG.md`. **Never release:** do not bump `BILOBA_VERSION`, do not edit a released section of `CHANGELOG.md`, do not tag. Onsi releases via the Release workflow (see `CLAUDE.md`).
