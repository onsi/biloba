...
- **A variadic `expected ...any` matcher must branch on `len(expected) == 0` explicitly — never default the missing case to a `nil` sub-matcher.** `matcherOrEqual(nil)` produces `gomega.BeNil()`, which asserted against a struct or `float64` can never pass; that shape (`matcherOrEqual(firstOrNil(expected))`) shipped on `HaveGapBetween`/`HaveOffsetTopWithin`/`HaveOffsetLeftWithin` for a while before anyone noticed the zero-arg form was permanently broken (issue #13) — the honest fix was giving `len(expected) == 0` its own branch with real presence semantics, matching every other variadic matcher in the codebase, not rejecting the call as a `Fatalf`.
