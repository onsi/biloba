...
- **`Capture` returns a new matcher and leaves the receiver alone**, so a matcher held in a variable can be reused with different targets (`m.Capture(&idA)`, then `m.Capture(&idB)`). `CookieMatcher.Capture` copies the same way, and `withField` carries `captureTarget` forward onto each refinement, so `.Capture` and the `.With*` chain compose in either order. A new builder-style matcher follows that shape.
...
Run `make test`. The full set of canonical invocations, and when to reach for each, is in the `biloba-testing` skill.
...
- Add a brief entry under `## Unreleased` in `CHANGELOG.md`. **Never release:** do not bump `BILOBA_VERSION`, do not edit a released section of `CHANGELOG.md`, do not tag. Onsi releases via the Release workflow, which moves `## Unreleased` into a versioned section and bumps the version (see `CLAUDE.md`).
