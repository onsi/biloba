...
| Shape | Means | Do |
|---|---|---|
| **flat** (one row, `held ×N`) | the product computed the value once and never reconciled | fix the product (`flaky-specs` §4) — a wider timeout won't help |
...
### Visual diagnosis (a failed `b.HaveScreenshot`) → `visual-assertions`
...
| `frame_detached: the document this frame handle was found in is gone` | the frame handle's iframe was removed or navigated, or its tab navigated or `Prepare()`d, since `b.Frame` found it | a handle belongs to one document: call `b.Frame(...)` again after whatever replaces the frame, and don't carry a handle across that change |
...
- A two-axis getter timing out because the **property** never became defined says the element was present, names the property, and prints the `b.AllowMissing("disabled")` to paste (`flaky-specs` §5).
...
Keep the `?? null` so a crashed page doesn't turn the snapshot itself into a failure. (That's a *snapshot*. To **wait** on app state as part of a spec, use `b.GetJSValue` — `flaky-specs` §3.)
...
- `BilobaConfigScreenshotTolerance(fraction)` / `BilobaConfigScreenshotChannelTolerance(delta)` — suite-wide visual-comparison defaults, both `0` (exact) by default; `b.Tolerance(...)`/`b.ChannelTolerance(...)` override per assertion. → `visual-assertions`
