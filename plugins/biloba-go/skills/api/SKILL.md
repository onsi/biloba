...
Selectors: CSS strings, `XPath` (`xpath`), or `Locator`s. `b.Immediate()` opts out of poll-by-default; rarely needed.
...
- `b.GetOffsetTopWithin(selector, container)` → float64 (`element.top - container.top`) · `b.GetOffsetLeftWithin` · `b.HaveOffsetTopWithin(container[, value|matcher])` · `b.HaveOffsetLeftWithin(...)`. No-arg form is existence-only: passes once both selector and container are laid out, without asserting on the offset value (and still captures it).
...
- `b.GetGapBetween(selector, other)` → `BoxDelta{Top,Left,Bottom,Right,Width,Height,CenterX,CenterY}` (subject minus other; `CenterX~0` ⇒ shared center line, `Width~0` ⇒ same size) · `b.HaveGapBetween(other[, value|matcher])`. No-arg form is existence-only: passes once both elements are laid out, without asserting on the delta (and still captures it).
...
- `b.Run(script[, &ptr])` → any — a synchronous **expression**; returns the decoded value. A top-level `return` is illegal (errors with a hint pointing at `RunAsync`/IIFE). Pollable: `Eventually(b.Run).WithArguments(expr).Should(matcher)` — it's a `func(string,...any) any`. → `flaky-specs`
...
- `b.ModifyResponse(url string|matcher)` → `.WithStatus(s).WithHeader(n,v).WithBody(b)` or `.Using(func(biloba.InterceptedResponse) biloba.StubResponse)` — rewrite the real response (reads real status/headers/body; heavier — pauses twice). A body Chrome refuses to hand over (a redirect's) or otherwise fails to read is never handed to `Using`/the static override — Biloba continues it unmodified (or fails the request, for a genuine read failure); `Count()` still ticks. → `debug-failures`
...
- **All handlers share one ordered, first-match-wins list.** In an `Ordered` container `Prepare()` is `OncePerOrdered`, so handlers **accumulate** across the `It`s: an earlier spec's handler permanently claims that URL and a later identical one is silent dead code. → `flaky-specs` §6
...
## Visual regression → `visual-assertions`
