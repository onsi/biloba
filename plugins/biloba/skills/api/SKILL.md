...
- `b.ModifyResponse(url string|matcher)` → `.WithStatus(s).WithHeader(n,v).WithBody(b)` or `.Using(func(biloba.InterceptedResponse) biloba.StubResponse)` — rewrite the real response (heavier — pauses twice). `InterceptedResponse` carries `URL`, `Status`, `Headers`, `Body`; `URL` is what tells the responses apart when the handler was registered with a matcher.
...
- `b.ViewportOnly()` → `*Biloba` — a clone-with-a-flag view (like `b.Realistic()`) whose **page** captures contain only what's on screen, at the current scroll position, instead of the whole document. `b.ViewportOnly().CaptureScreenshot()`, `Expect(b.ViewportOnly()).To(b.HaveScreenshot("fold"))`. Combining it with an **element** capture is a hard error, not a silent no-op — an element capture is already clipped to its box.
...
- `b.VisualComparisons()` → `[]biloba.VisualComparison` — what each `HaveScreenshot` comparison **measured** (the prose diagnosis as data), as opposed to the files it wrote. Snapshot; cleared by `Prepare()`. See the visual-regression section below.
...
- `b.VisualComparisons()` → `[]biloba.VisualComparison` — the diagnosis as data, for a reporter or an assertion on *how* an image changed. Carries `Name`/`Label`/`ColorScheme`, `Match`, `MissingBaseline` (its own verdict — nothing was compared, every measurement is zero, the fix is to generate baselines), the three paths, `BaselineSize`/`ActualSize`/`DimensionMismatch`, the `Tolerance`/`ChannelTolerance` it ran under, `TotalPixels`/`DifferingPixels`/`Fraction`/`MaxChannelDelta`, and the shape verdicts `Regions`/`RegionCount`/`Shifted`/`Shift`/`Scattered`. **The measurements are definitions; the verdicts are tuned heuristics whose thresholds move** — assert `RegionCount == 1`, not `== 7`. Passing comparisons are recorded too, so an empty list means "asserted nothing visually", not "everything passed"; only the attempt that *decided* an assertion is recorded — one entry per scheme `InColorSchemes` actually measured (a failure stops at the first failing scheme).
