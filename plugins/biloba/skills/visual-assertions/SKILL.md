...
**The same diagnosis as data.** `b.VisualComparisons()` returns a `[]biloba.VisualComparison` — the measurements of the attempt that *decided* the assertion, passing ones included, one entry per scheme `InColorSchemes` actually measured. It carries the paths, `Match`, `MissingBaseline`, `BaselineSize`/`ActualSize`/`DimensionMismatch`, the `Tolerance`/`ChannelTolerance` it ran under, `TotalPixels`/`DifferingPixels`/`Fraction`/`MaxChannelDelta`, and the verdicts `Regions`/`RegionCount`/`Shifted`/`Shift`/`Scattered`. Reach for it in a `ReportAfterEach` that records regressions, or when a spec wants to assert on *how* an image changed. **Don't parse the prose above for these numbers.** And mind which half you assert on: the measurements are definitions, the verdicts are tuned heuristics whose thresholds move — `RegionCount == 1` is fair, `== 7` breaks on an improvement.
