...
- The docs and the `biloba:setup` skill now cover a suite-level cost that has nothing to do with Biloba's own overhead: one `httptest.NewServer` per spec puts every spec on a new ephemeral port, so every asset URL changes and the renderer starts cold on each navigation. Measured on one real suite (a 1.67 MB bundle), a stable origin was worth ~31ms per `b.Navigate` and ~20s off a 1,558-spec `--procs=6` run. It is not the HTTP cache - disabling that on a stable origin costs nothing. Keep one server per parallel process, or bind a fresh per-spec server to the same port; per-spec state can stay fresh either way, since only the origin has to be shared. If you pin a port, give each fixture its own `&http.Transport{}`: `http.DefaultTransport`'s pool is process-global and keyed on `host:port`, so a spec's first request can be handed a keep-alive socket to the previous spec's closed server.
- The docs no longer claim the poll-trajectory artifact costs "a few nanoseconds per poll" - it takes a lock and renders the observed value per sample, so that was asserted rather than measured. Replaced with a real number: on a 1,558-spec suite with 2,470 call sites across the instrumented matchers, turning recording off saved under a second across the whole run, inside that suite's own run-to-run spread. Leave it on.
- `biloba.js` is now installed by Chrome at the start of every document, rather than by Biloba after it notices the previous copy is gone. Each tab registers the script once (`Page.addScriptToEvaluateOnNewDocument`), so `window._biloba` is in place before any of the page's own scripts run - in the main frame and in every same-origin child frame. This closes a real window: a page that navigated itself between Biloba's "is it installed?" check and the command that followed left that command looking at a document which never had the bridge, recoverable only by matching on a "_biloba is not defined" error string and reinstalling. That repair path is still there as a safety net, and a tab whose registration does not take falls back to the old reinstall-after-each-navigation behaviour, so nothing depends on the new mechanism silently working. A navigation also no longer re-sends the script, which saves about 2ms per navigation.
- A note for apps that sandbox untrusted content: the every-document install covers child frames, but whether it reaches a frame with an OPAQUE origin (`sandbox="allow-scripts"` without `allow-same-origin`) depends on the Chrome build - `chrome-headless-shell` injects into one, full headless Chrome does not. Either way the frame keeps running its own scripts and keeps talking to its parent, and Biloba never uses a copy there (`>>>` cannot pierce an opaque origin by design), so nothing depends on which way it lands.

## 0.15.3

- Biloba no longer hangs when Chrome stops responding. Every command it sends the browser now runs under a backstop deadline (30s; 2 minutes for `RunAsync`, whose duration is set by the promise your page awaits), so a wedged Chrome fails the spec instead of blocking a worker until the whole suite times out. `WithTimeout` could not cover this: it bounds Gomega's `Eventually` loop, but a command that never returns blocks inside the poll's callback where Gomega cannot interrupt it. The backstop is not a new knob - you do not configure it, and `WithTimeout` does not shorten it - and it applies in every bucket, including the snapshots and one-shot mutations that reject every knob.
- Failures from an unresponsive browser now say why: `deadline_exceeded` (Chrome accepted the command and never answered), `page_crashed` (Chrome reported the renderer crash - recoverable, navigating gives it a fresh renderer), and `browser_gone` (the browser process is no longer there).  Which of the first two you get is Chrome's call: it announces a crash promptly on macOS and several seconds later on Linux, so a command in between reports `deadline_exceeded`.  Both are bounded and both name a cause.  Note that a crashed tab is not dependably recoverable - it comes back on a navigation on macOS, and in our testing stays dead on Linux - so continue on a fresh `b.NewTab()` rather than the crashed one.
- Note for anyone [dropping to chromedp](https://onsi.github.io/biloba/#codechromedpcode-breaking-the-fourth-wall): `b.Context` still carries no deadline. Wrap your own `chromedp.Run` in a `context.WithTimeout` derived from `b.Context`.
- `Prepare()` no longer attributes the previous spec's `beforeunload` dialog to the next spec.  Its closing `about:blank` navigation unloads the page the last spec left behind; the dialog that raises is now accepted silently instead of appearing in the new spec's `Dialogs()` and printing an "unhandled dialog" warning against it.
- `DragTo(source, target)`'s "target not found" failure now names the missing target selector instead of the source selector (which was found).
- `HaveGapBetween`, `HaveOffsetTopWithin`, and `HaveOffsetLeftWithin` now have a real zero-arg (existence-only) form: calling them with no expected value used to compile but always fail (it asserted the `BoxDelta`/offset was `nil`). Now the zero-arg form passes once the geometry resolves - both elements laid out, or the element laid out within its container - without asserting on the value, and you can still `.Capture` it. This matches the zero-arg presence semantics `HaveProperty`/`HaveAttribute` already had.
- The `Poll trajectory` failure artifact now covers the DOM value matchers (`HaveInnerText`/`HaveTextContent`/`HaveProperty`, `HaveValue`, `HaveClass`, `HaveText`, `HaveAttribute`, `HaveCount`, `HaveDistinctCount`, `HaveComputedStyle`, `HaveJSONAttribute`, the `Each*` forms) and `EvaluateTo` - the assertions specs actually write most, which previously got no trajectory at all.
- A trajectory is now attached only to the assertion that actually failed. It used to render whichever series was most recent, so a read that had already *passed* - or a `b.Run` setup line - could be attached to an unrelated later failure, under the "value never changed: the page is not re-evaluating this probe" diagnosis. A failure with no polled value read behind it now gets no entry rather than someone else's. `b.Run`/`b.RunAsync` no longer record (they are one-shot mutations, not polls); poll an expression with `b.EvaluateTo` or `b.GetJSValue` to get a trajectory for it.
- A single-sample series no longer claims the "value never changed" diagnosis - one sample cannot tell that apart from "only read once".
- `b.Realistic().DragTo(source, target)` can now drag between two rows of the same scroll container. It used to scroll and measure each endpoint in turn, so scrolling the target into view moved the source out from under the coordinate already recorded for it - and for two rows of one `overflow: auto` list both coordinates landed on the pane's center, where the press and release arrive together and the page receives a click on the target rather than a drag. Both endpoints are now measured at a single scroll position: Biloba tries where things already are (the common case, and it disturbs nothing), then the pair framed around their midpoint, then each endpoint scrolled in with the other scrolled second, which is what endpoints in two separate panes need.
- When no scroll position can show both drag endpoints - rows further apart than their pane is tall - realistic `DragTo` now fails and names both endpoints, along with why each one is unreachable (off-screen, obscured, or scrolled out of view inside the pane it names). It used to dispatch the drag anyway. The failure is retryable, so a list still settling still gets there; the diagnosis appears in the timeout message.
- The docs and the `biloba:realistic-mode` skill now name a fast-vs-realistic asymmetry worth knowing when you switch tracks: a `position: fixed` footer is invisible to the fast click track (`element.click()` does no hit-testing) and perfectly solid to real pointer input, so a spec that passed for months can fail the moment it goes realistic. Both tracks are correct; the realistic one is reporting that a user couldn't reach the element either.
- The visual-regression partial-clip warning now has a documented triage step (docs and the `biloba:visual-assertions` skill): compare the subject's height against the clipping pane's visible band to tell "the subject is taller than its pane", which is a fact about the design and will warn forever, from "the pane is scrolled somewhere else", which `b.ScrollIntoView(subject, b.WithinScroller(pane))` fixes.
- The docs and the `biloba:realistic-mode` skill now say that `DragTo` is the whole gesture - press, moves and release inside one call - so a spec asserting on the *mid-drag* state (a tree row that auto-expands on a hover dwell, a drop-target highlight, a spring-loaded folder) drops to the three CDP legs on `b.Context`, with a worked example. `b.GetBoundingBox` already reports in the coordinate space CDP mouse events use, so `CenterX`/`CenterY` are the coordinates to send.
