...
- **The plugin skills are essential project surface — keep them current.** The skills under `.claude/skills/` (`biloba-dom-method`, `biloba-testing`) are how future work discovers conventions, and they go stale silently (e.g. the 0.6 keyboard modifiers shipped without a skill update). Whenever a change adds or alters a method family, an option, a convention, or the test/release flow, update the relevant skill in the **same** change — treat it like docs, not an afterthought. When in doubt, update the skill.
