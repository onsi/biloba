...
- Keep docs in sync by client: Go/Ginkgo/Gomega behavior belongs in `docs/index.md` and godoc comments; TypeScript/Vitest behavior belongs in `docs/vitest.md` and `typescript/README.md`. Shared engine or protocol behavior may require both. Stage user-facing changes in `CHANGELOG-TMP.md` as you go (keep entries brief).
- **Three load-bearing skill surfaces — keep the affected ones current.** (1) Repo skills under `.claude/skills/` drive work on Biloba itself. (2) `plugins/biloba-gomega/skills/` ships Go/Ginkgo/Gomega guidance. (3) `plugins/biloba-vitest/skills/` ships TypeScript/Vitest guidance. The deprecated `plugins/biloba/skills/` namespace contains only symlinks to `biloba-gomega`; never put an independent skill file there. Keep canonical Gomega skill-to-skill references namespace-neutral so the same file works through either prefix. Update only the client(s) whose public behavior changed; if engine/protocol behavior is observable through both clients, update both plugins in the same change. Never route a Gomega skill to a Vitest skill or vice versa. Within a plugin, facts in `write-tests`, `overview`, diagnostics/flake guidance, and any API reference must agree. Run `make check-plugins` before handing off.
- **Never release.** Onsi releases, using a `shipit` binary you must never run. Do not bump `BILOBA_VERSION`, do not edit `CHANGELOG.md` (the released log), and do not tag/publish. Your job ends at staging notes in `CHANGELOG-TMP.md`; `shipit` folds those into `CHANGELOG.md`, bumps the version, then must run `make sync-plugin-versions` so all three plugin manifests receive that version before the release commit. `make check-plugins` rejects a partial bump.
