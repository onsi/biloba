...
- **Three skill surfaces — keep the affected ones current.** (1) Repo skills under `.claude/skills/` drive work on Biloba itself. (2) `plugins/biloba-go/skills/` ships Go/Ginkgo/Gomega guidance. (3) `plugins/biloba-vitest/skills/` ships TypeScript/Vitest guidance. The deprecated `plugins/biloba/skills/` namespace contains only symlinks to `biloba-go`; never put an independent skill file there. Keep canonical Gomega skill-to-skill references namespace-neutral so the same file works through either prefix. Update only the client(s) whose public behavior changed; if engine/protocol behavior is observable through both clients, update both plugins in the same change. Never route a Gomega skill to a Vitest skill or vice versa. Within a plugin, facts in `write-tests`, `overview`, diagnostics/flake guidance, and any API reference must agree. Run `make check-plugins` before handing off.
- **Never release.** Onsi releases by running the Release workflow (`release.yml`, a `workflow_dispatch` with a patch/minor input): it tests, moves `## Unreleased` into a versioned section, bumps `BILOBA_VERSION`, runs `make sync-plugin-versions` (three plugin manifests plus `typescript/package.json`), tags, creates the GitHub release with `bilobad` binaries, and publishes the npm packages. Your job: add brief entries under `## Unreleased` in `CHANGELOG.md` (under `### Features` or `### Fixes`). Never edit a released section, never bump `BILOBA_VERSION`, never trigger the Release workflow (button or `gh workflow run`), and never tag, create a release, or `npm publish`.
