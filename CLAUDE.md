...
1. **Performance via parallelization.** One shared Chrome process; each Ginkgo parallel process drives its own isolated *root tab* (`b`), reused between specs via `b.Prepare()`. Creating tabs is cheaper than creating browsers; reusing a tab is cheaper than creating one.
...
- **Frames.** `>>>` reaches same-origin iframes. A cross-origin iframe gets a *frame handle* — `b.Frame(query)` in Go, `session.waitForFrame()` in TypeScript — scoped to one frame document: evaluation is pinned to that document's JavaScript context (`engine.ScopeToFrameContext`), and a handle whose document is gone fails as stale rather than acting on its replacement. Same-process frames share their tab's renderer attachment, so pointer points and screenshot clips are translated into the tab (`engine/frame_geometry.go`, used by both clients) and tab-level commands are refused on a frame (`refusedOnFrame` in Go, `tabOnly` in the engine) — otherwise they would act on the embedding page. Out-of-process frames (full Chrome runs cross-site frames in their own target) are reached through an attachment to that target (`engine.AttachFrameTargetContext`). A frame feature lands in both clients together.
...
- **TypeScript client.** `typescript/` (the `biloba` npm package) spawns one `bilobad` (`cmd/bilobad/`) per Vitest worker and speaks the framed JSON protocol in `protocol/`, whose TypeScript types are generated from the Go wire structs (`go generate ./protocol`). `bilobad` drives Chrome through `engine/`, the runner-neutral core. `engine/biloba.js` is a generated copy of `biloba.js`: run `go generate ./engine` after editing `biloba.js`, so a DOM change reaches both clients.
- **chromedp escape hatch.** Every tab exposes `b.Context`; Biloba deliberately does not hide chromedp/cdproto. Missing Go features (e.g. geolocation, which the engine and TypeScript client support but the Go API does not expose yet) are expected to be done through `b.Context` until/unless Biloba grows native support.
...
Matchers are built with `gcustom.MakeMatcher` and return `(bool, error)` (commonly via `bilobaJSResponse.MatcherResult()`), using `.WithMessage`/`.WithTemplate` for failure output. A matcher that *reads a value* off the page returns the concrete `*ValueMatcher` (`capture.go`) instead of `types.GomegaMatcher`, which is what gives it `.Capture(&x)` — the value from the winning read, so a spec never has to gate-then-re-read. Matchers that read no value, and the actions-as-matchers, keep the bare interface so `.Capture` does not compile on them; that distinction *is* the API. While polling, both `(false, nil)` (not ready) and `(false, err)` (genuine JS error) retry — errors surface inside Gomega's "Timed out after…" message at the deadline; only `Immediate()` fails fast on them.
...
**Every Go test is a Ginkgo spec** (the TypeScript client's tests under `typescript/` are Vitest). The `Makefile` wraps the canonical invocations: `make test` (standard headless parallel run — `ginkgo -r -p --randomize-all --randomize-suites`), `make test-all` (also runs the high-fidelity google-chrome lane CI uses), and `make stress-test` (6 procs under `stress` load, 41 repeats — for flushing out timing/concurrency races). `make driver-test`, `driver-parity`, `driver-e2e`, and `packaging-check` cover `engine/`, `protocol/`, `bilobad`, the TypeScript client, and the npm packages. Run `make stress-test` only periodically or when you suspect a change might be flaky; see the `biloba-testing` skill for details.

```
make test     # or: ginkgo -r -p --randomize-all --randomize-suites
```
...
| Cross-origin iframes (`Frame`/`HaveFrame`/`AllFrames`, frame handles, tab-only refusals) | `frames.go`; engine: `engine/frame.go`, `engine/frame_context.go`, `engine/frame_geometry.go` | `frames_test.go`; `engine/frame*_test.go` |
...
The backlog is GitHub issues on `onsi/biloba`; the `TODO` file predates them and is historical.

## Conventions

- Keep docs in sync by client: Go/Ginkgo/Gomega behavior belongs in `docs/index.md` and godoc comments; TypeScript/Vitest behavior belongs in `docs/vitest.md` and `typescript/README.md`. Shared engine or protocol behavior may require both. Add user-facing changes under `## Unreleased` in `CHANGELOG.md` as you go (keep entries brief).
- **Three skill surfaces — keep the affected ones current.** (1) Repo skills under `.claude/skills/` drive work on Biloba itself. (2) `plugins/biloba-go/skills/` ships Go/Ginkgo/Gomega guidance. (3) `plugins/biloba-vitest/skills/` ships TypeScript/Vitest guidance. Gomega skills name their siblings without a plugin prefix (and carry the note saying to reuse the prefix they were loaded under); Vitest skills use the `biloba-vitest:` prefix. Update only the client(s) whose public behavior changed; if engine/protocol behavior is observable through both clients, update both plugins in the same change. Never route a Gomega skill to a Vitest skill or vice versa. Within a plugin, facts in `write-tests`, `overview`, diagnostics/flake guidance, and any API reference must agree. Run `make check-plugins` before handing off.
- **Never release.** Onsi releases by running the Release workflow (`release.yml`, a `workflow_dispatch` with a patch/minor input): it tests, moves `## Unreleased` into a versioned section, bumps `BILOBA_VERSION`, runs `make sync-plugin-versions` (both plugin manifests plus `typescript/package.json`), tags, creates the GitHub release with `bilobad` binaries, and publishes the npm packages. Your job: add brief entries under `## Unreleased` in `CHANGELOG.md` (under `### Features` or `### Fixes`). Never edit a released section, never bump `BILOBA_VERSION`, never trigger the Release workflow (button or `gh workflow run`), and never tag, create a release, or `npm publish`.
...
- On-failure artifacts adapt to human vs CI/agent environments; the rules and test seams are in the `biloba-testing` skill.
