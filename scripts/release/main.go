...
// the repository root:
//
//	go run ./scripts/release current            print BILOBA_VERSION
//	go run ./scripts/release next patch|minor   print the version a bump would produce
//	go run ./scripts/release prepare X.Y.Z      release ## Unreleased as ## X.Y.Z and set BILOBA_VERSION
...
func readVersion() (string, error) {
	source, err := os.ReadFile(bilobaGoPath)
	if err != nil {
		return "", err
	}
...
func prepare(version string) error {
	...
	return os.WriteFile(bilobaGoPath, []byte(newSource), 0o644)
}
